...
<div class="my-container-3 sectors-holder">
	<div class="row sector-header">
		...
		<c:if test="${debug}">
			<div class="btn-group">
				<a href="../data/relservhoteldocdocuments?initLists=true&initDicts=true" 		target="_blank" class="btn btn-default"> JSON DATA </a>
				<a href="../data/relservhoteldocdocumentsview?initLists=true&initDicts=true" 	target="_blank" class="btn btn-default"> JSON DATA FOR VIEW </a>
			</div>
		</c:if>
		</div>
		<div class="without-padding col-md-12">
			<div class="has-feedback filterBar">
				<input type="text" ng-model="searchKeywordRelServHotelDocDocuments" class="form-control"
					   ng-model-options="{ debounce: 300 }" ng-change="search('filteredRelServHotelDocDocuments', 'dataRelServHotelDocDocuments', searchKeywordRelServHotelDocDocuments)"
	...
	<div class="row sector-1">
		<div class="col-md-12 without-padding scrollable-content-65">
		  <div class="table-responsive margin-bottom-5 table-body" when-scrolled="loadDataRelServHotelDocDocuments()">
			<table class="table table-hover table-striped table-bordered margin-bottom-5 padding-all-2">
				...
				<tbody>
					<!-- <tr ng-repeat="item in filteredRelServHotelDocDocuments = ( dataRelServHotelDocDocuments | filter:searchKeywordRelServHotelDocDocuments | orderBy: details.sortableColumn: details.sortableReverse )" -->
					<tr ng-repeat="item in filteredRelServHotelDocDocuments track by item.id"
		...
		<div class="col-md-12 margin-bottom-10">
			<div class="btn-toolbar">
				<div class="btn-group" dropdown>
					<button type="button" class="btn btn-default" ng-click="searchKeywordRelServHotelDocDocuments = ''">
						<i class="fa fa-list-alt fa-lg"></i>
						<span> <spring:message code="default.total" />: {{ filteredRelServHotelDocDocuments.length }} / {{ dataRelServHotelDocDocuments.length }} </span>
					</button>
					...
					<ul class="dropdown-menu" role="menu">
						<li><a ng-click="searchKeywordRelServHotelDocDocuments = ''"><spring:message code="default.ShowAll" /></a></li>
						<li><a ng-click="sort('createon', false)"><spring:message code="default.ShowLast" /></a></li>
					</ul>
				</div>
				<div class="btn-group" dropdown>
					<button type="button" class="btn btn-primary" ng-click="loadDataRelServHotelDocDocuments()">
					...
					<ul class="dropdown-menu" role="menu">
						<li><a ng-click="loadDataRelServHotelDocDocuments()"><spring:message code="default.loadMore" /></a></li>
						<li><a ng-click="loadAllDataRelServHotelDocDocuments()"><spring:message code="default.loadAll" /></a></li>
					</ul>
				</div>
  			</div>
		</div>
