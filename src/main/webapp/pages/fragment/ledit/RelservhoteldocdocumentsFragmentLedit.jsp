	...
	<div class="row padding-left-right-15">
		<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
			<button type="button" class="btn btn-default btn-create"
				ng-click="createModalItemRelServHotelDocDocuments()">
				<i class="fa fa-file-o pull-left"></i>
				<i class="fa fa-file pull-left"></i>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.create" /></span>
			</button>
			<button type="button" class="btn btn-default btn-edit"
				ng-click="uploadRelServHotelDocDocuments()">
			...
			<button type="button" class="btn btn-default btn-reload"
				ng-click="pickItemRelServHotelDocDocuments()">
		...
		<div class="col-md-12">
			<div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered margin-bottom-5 padding-all-2">
					...
					<tbody>
						<tr ng-repeat="item in item.relServHotelDocDocumentses track by $index"
							ng-class="{ 'success' : selectedItemrelServHotelDocDocumentses.id == item.id }"
							ng-dblclick="editModalItemRelServHotelDocDocuments(item.id)"
							ng-click="setSelected('selectedItemrelServHotelDocDocumentses', item, $index)">
						...
						<td ng-if="::checkVisibility('servhotel')"><label>{{ ::item.servhotel.label }}</label></td>
							<td>
								<a ng-click="editModalItemRelServHotelDocDocuments(item.id)" class="btn">
									<i class="glyphicon glyphicons-lg glyphicon-pencil"></i> 
								</a>
							</td>
							<td>
								<a ng-click="deleteSubItem('relServHotelDocDocumentses', 'RelServHotelDocDocuments', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.relServHotelDocDocumentses.length }}
				</div>
			</div>
		</div>
	</div>
