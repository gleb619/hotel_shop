	...
	<div class="row padding-left-right-15">
		<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
			<button type="button" class="btn btn-default btn-reload"
				ng-click="pickItemRelTmOrderTmContact()">
				<i class="glyphicon glyphicon-list-alt pull-left"></i>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.pick" /></span>
			</button>
			<button type="button" class="btn btn-default btn-delete"
				ng-click="deleteSubItem('relTmOrderTmContacts', 'RelTmOrderTmContact', selectedItemrelTmOrderTmContacts.id, itemIndex)">
		...
		<div class="col-md-12">
			  <div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in item.relTmOrderTmContacts track by $index"
							ng-class="{ 'success' : selectedItemrelTmOrderTmContacts.id == item.id }"
							ng-click="setSelected('selectedItemrelTmOrderTmContacts', item, $index)">
						...
						<td ng-if="::checkVisibility('tmcontact')"><label>{{ ::item.tmcontact.label }}</label></td>
							<td>
								<a ng-click="deleteSubItem('relTmOrderTmContacts', 'RelTmOrderTmContact', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.relTmOrderTmContacts.length }}
				</div>
			  </div>
		</div>
	</div>
