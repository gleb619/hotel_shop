	...
	<div class="row padding-left-right-15">
		<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
			<button type="button" class="btn btn-default btn-reload"
				ng-click="pickItemRelServHotelFeatures()">
				<i class="glyphicon glyphicon-list-alt pull-left"></i>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.pick" /></span>
			</button>
			<button type="button" class="btn btn-default btn-delete"
				ng-click="deleteSubItem('relServHotelFeatureses', 'RelServHotelFeatures', selectedItemrelServHotelFeatureses.id, itemIndex)">
		...
		<div class="col-md-12">
			  <div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in item.relServHotelFeatureses track by $index"
							ng-class="{ 'success' : selectedItemrelServHotelFeatureses.id == item.id }"
							ng-click="setSelected('selectedItemrelServHotelFeatureses', item, $index)">
						...
						<td ng-if="::checkVisibility('servhotel')"><label>{{ ::item.servhotel.label }}</label></td>
							<td>
								<a ng-click="deleteSubItem('relServHotelFeatureses', 'RelServHotelFeatures', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.relServHotelFeatureses.length }}
				</div>
			  </div>
		</div>
	</div>
