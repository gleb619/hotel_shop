	...
	<div class="row padding-left-right-15">
		<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
			<button type="button" class="btn btn-default btn-reload"
				ng-click="pickItemRelCruiseContacts()">
				<i class="glyphicon glyphicon-list-alt pull-left"></i>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.pick" /></span>
			</button>
			<button type="button" class="btn btn-default btn-delete"
				ng-click="deleteSubItem('relCruiseContactses', 'RelCruiseContacts', selectedItemrelCruiseContactses.id, itemIndex)">
		...
		<div class="col-md-12">
			  <div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in item.relCruiseContactses track by $index"
							ng-class="{ 'success' : selectedItemrelCruiseContactses.id == item.id }"
							ng-click="setSelected('selectedItemrelCruiseContactses', item, $index)">
						...
						<td ng-if="::checkVisibility('servcruise')"><label>{{ ::item.servcruise.label }}</label></td>
							<td>
								<a ng-click="deleteSubItem('relCruiseContactses', 'RelCruiseContacts', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.relCruiseContactses.length }}
				</div>
			  </div>
		</div>
	</div>
