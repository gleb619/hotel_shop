	...
	<div class="row padding-left-right-15">
		<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
			<button type="button" class="btn btn-default btn-reload"
				ng-click="pickItemRelServHotelCreditCards()">
				<i class="glyphicon glyphicon-list-alt pull-left"></i>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.pick" /></span>
			</button>
			<button type="button" class="btn btn-default btn-delete"
				ng-click="deleteSubItem('relServHotelCreditCardses', 'RelServHotelCreditCards', selectedItemrelServHotelCreditCardses.id, itemIndex)">
		...
		<div class="col-md-12">
			  <div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in item.relServHotelCreditCardses track by $index"
							ng-class="{ 'success' : selectedItemrelServHotelCreditCardses.id == item.id }"
							ng-click="setSelected('selectedItemrelServHotelCreditCardses', item, $index)">
						...
						<td ng-if="::checkVisibility('servhotel')"><label>{{ ::item.servhotel.label }}</label></td>
							<td>
								<a ng-click="deleteSubItem('relServHotelCreditCardses', 'RelServHotelCreditCards', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.relServHotelCreditCardses.length }}
				</div>
			  </div>
		</div>
	</div>
