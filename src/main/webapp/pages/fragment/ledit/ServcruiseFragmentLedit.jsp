		...
		<div class="padding-left-right-15">
			<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
				<button type="button" class="btn btn-default btn-create"
					ng-click="createModalItemServCruise()">
					<i class="fa fa-file-o pull-left"></i>
					<i class="fa fa-file pull-left"></i>
					<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.create" /></span>
				</button>
				<button type="button" class="btn btn-default btn-edit"
					ng-click="editModalItemServCruise(selectedItemservCruises.id)">
					<i class="glyphicon glyphicon-pencil pull-left"></i>
					<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.edit" /></span>
				</button>
				<button type="button" class="btn btn-default btn-delete"
					ng-click="deleteSubItem('servCruises', 'ServCruise', selectedItemservCruises.id, itemIndex)">
		...
		<div class="col-md-12">
			<div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered margin-bottom-5 padding-all-2">
					...
					<tbody>
						<tr ng-repeat="item in item.servCruises track by $index"
							ng-class="{ 'success' : selectedItemservCruises.id == item.id }"
							ng-dblclick="editModalItemServCruise(item.id)"
							ng-click="setSelected('selectedItemservCruises', item, $index)">
						...
						<td ng-if="::checkVisibility('cruisedescr')"><label>{{ ::item.cruisedescr }}</label></td>
							<td>
								<a ng-click="editModalItemServCruise(item.id)" class="btn">
									<i class="glyphicon glyphicons-lg glyphicon-pencil"></i> 
								</a>
							</td>
							<td>
								<a ng-click="deleteSubItem('servCruises', 'ServCruise', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.servCruises.length }}
				</div>
			</div>
		</div>
	</div>
