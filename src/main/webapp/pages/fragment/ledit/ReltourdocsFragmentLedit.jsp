	...
	<div class="row padding-left-right-15">
		<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
			<button type="button" class="btn btn-default btn-reload"
				ng-click="pickItemRelTourDocs()">
				<i class="glyphicon glyphicon-list-alt pull-left"></i>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.pick" /></span>
			</button>
			<button type="button" class="btn btn-default btn-delete"
				ng-click="deleteSubItem('relTourDocses', 'RelTourDocs', selectedItemrelTourDocses.id, itemIndex)">
		...
		<div class="col-md-12">
			  <div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in item.relTourDocses track by $index"
							ng-class="{ 'success' : selectedItemrelTourDocses.id == item.id }"
							ng-click="setSelected('selectedItemrelTourDocses', item, $index)">
						...
						<td ng-if="::checkVisibility('servtour')"><label>{{ ::item.servtour.label }}</label></td>
							<td>
								<a ng-click="deleteSubItem('relTourDocses', 'RelTourDocs', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.relTourDocses.length }}
				</div>
			  </div>
		</div>
	</div>
