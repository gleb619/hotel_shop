	...
	<div ng-if="!loading" class="edit-form-body">
		<div class="modal-header header">
			...
			<h4 class="modal-title">
				...
				<spring:message code="label.ServCruise" />
				<c:if test="${debug}">
					<span>{{ item.id }}</span>
					<a href="../data/servcruise/{{ ::item.id }}?initLists=true&initDicts=true" target="_blank" class="btn btn-default">
						JSON DATA
					</a>
				</c:if>
				<!--
		...
		<div class="modal-body padding-top-0">
			<tabset type="pills"> 
			  ...
			  <tab select="initTab('infoservices', 'fragmentledit')">
					...
					<tab select="initTab('relcruisedocs', 'fragmentledit')">
						<tab-heading><spring:message code="label.RelCruiseDocs"/></tab-heading>
						
						<div ng-if="templates.relcruisedocs.fragmentledit.init" ng-include="templates.relcruisedocs.fragmentledit.url"></div>
					</tab>
